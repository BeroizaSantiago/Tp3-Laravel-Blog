                    ...
                    @if(Auth::check())
                        @if($post->user_id === auth()->id())
                            <div class="absolute bottom-6 pt-8">
                                <a href="{{ route('category.delete', $post->id) }}"><button type="button" class="text-white bg-red-600 p-2 mr-1 rounded w-20">Eliminar</button></a>
                                <a href="{{ route('category.edit', $post->id) }}"><button type="button" class="text-white bg-green-700 p-2 rounded w-20">Editar</button></a>
                            </div>
                        @endif
                    @endif
                @endif
